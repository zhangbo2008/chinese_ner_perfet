...
class BartEncoder(nn.Module):
    ...
    def forward(
        ...
        # T x B x C -> B x T x C
        x = x.transpose(0, 1)
# all_attention在encoder中的注意力是英文和英文之间的注意力.
        if not return_dict:
...
# 先计算中文和中文之间的注意力.
        ...
        # Cross attention#================核心难点.
        residual = x
        assert self.encoder_attn.cache_key != self.self_attn.cache_key
        if self.normalize_before:
            x = self.encoder_attn_layer_norm(x)
        x, en_de_atten = self.encoder_attn(  # 这部分是计算encoder-decoder注意力的计算.  en_de_atten 是经过softmax\之后的注意了!!!!!!!!!!!!!!!!!!
            query=x,
    ...
    def forward(
        ...
        **unused,
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            x, layer_self_attn, layer_past,encoder_decoder_atten = decoder_layer(
                x, # x 是 中文句子.
                encoder_hidden_states, # 英文句子
        ...
        if not return_dict:
            return tuple(v for v in [x, next_cache, all_hidden_states, all_self_attns,encoder_decoder_atten] if v is not None)
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        attn_output = torch.bmm(attn_probs, v) # 2020-10-11,13点34 最新悟到的transformer结构, 输入q,k,v 输出 一个 q的新表示.还是13维度.这个新的q, 就包含了词语之间互相的注意力表示.
        ...
        if output_attentions:
            ...
            attn_weights = None
        return attn_output, attn_weights  # attn_output 是编码结构.!!!!!!!!!!!!!!!!注意这里面的注意力已经是softmax之后的, 所以他们和是1.不要再softmax了.
