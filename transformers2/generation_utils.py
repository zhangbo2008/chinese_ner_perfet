...
#-----------------绝对的核心代码!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            ...
            # print(src_texts,3333333333333)
            # print(mydic,9999999999999)
            # print(tokenizermy,9999999999999)
            outputs = self(**model_inputs, return_dict=True)
            next_token_logits = outputs.logits[:, -1, :] # outputs.logits 输出在字典每一个token上的概率分布.
#-----------才是后处理函数!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            #         # 先是不在句首.
            #         if jushou == 0:  # 如果不在句首,
            #             mini = tokenizer.prepare_seq2seq_batch(src_texts=['apple'], tgt_texts=['驢' + tgt_ner])
            #             # 去掉首位置的8,末尾位置的0.
            #             # 进行筛选掉!
            #         else:
            #             mini = tokenizer.prepare_seq2seq_batch(src_texts=['apple'], tgt_texts=[tgt_ner])
            #
            #         src_ner_id = [i1 for i1 in mini.data['input_ids'][0] if i1.item() not in [8, 0, 60407]]
            #         tgt_ner_id = [i1 for i1 in mini.data['labels'][0] if i1.item() not in [8, 0, 60407]]
